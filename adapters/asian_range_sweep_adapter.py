"""
Asian Range Sweep Strategy Adapter

Handles Asian session range detection and sweep/extension logic.
"""
...
class AsianRangeSweepAdapter(BaseAdapter):
    """
    Adapter for Asian Range Sweep strategy.

    Strategy specifics:
    - Asian session range detection (typically 0000-0800 UTC)
    - Sweep of Asian range (breakout beyond range)
    ...
    def validate_config(self, config: Any) -> bool:
        """Validate Asian Range Sweep config has required attributes."""
        required_attrs = [
            'asian_session_start',
            'asian_session_end',
            'range_sweep_threshold',
            'tp_r', 'init_cash', 'fees'
        ]
    ...
    def preprocess_signals(
        ...
        """
        Preprocess signals for Asian Range Sweep strategy.

        Asian Range Sweep may require filtering entries based on
        confirmed sweeps beyond the Asian range. This can be done here.
        """
    ...
    def create_portfolio(
        ...
        """
        Create portfolio for Asian Range Sweep strategy.

        Asian Range Sweep can use from_signals if entry logic is handled
