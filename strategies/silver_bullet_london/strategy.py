"""
Silver Bullet (London) Strategy Implementation

Silver Bullet window during London killzone (03:00–04:00 NY).
Same logic as NY AM but different time window.

Strategy Specification from ICT Unified Trading Book:
- Time: 03:00–04:00 NY (London killzone)
- Session: London Open
...
from config import SilverBulletLondonConfig
from backtest_agent import get_order_func_state_tracker


def generate_signals(price_data: pd.DataFrame, cfg: SilverBulletLondonConfig) -> Tuple[pd.Series, pd.Series]:
    """
    Generate entry and exit signals for Silver Bullet (London) strategy.
    ...
    # London killzone mask (03:00-04:00)
    killzone_mask = in_killzone(price_data.index, "london_open")
...
def order_func(context, cfg):
    """
    Custom order function for Silver Bullet (London) with partial exits and breakeven trailing.
...
def run_backtest(cfg: SilverBulletLondonConfig) -> vbt.Portfolio:
    """
    Run backtest for Silver Bullet (London) strategy.
...
if __name__ == "__main__":
    ...
    cfg = SilverBulletLondonConfig()
    cfg.start_date = "2024-01-01"  # Optional date filtering
    cfg.end_date = "2024-12-31"
    
    agent = BacktestAgent("silver_bullet_london", "1.0")
    
    print(f"Silver Bullet (London) Strategy Backtest")
